...
async function browse(p=current){current=p;q('#crumb').textContent='/'+p;q('#history').classList.add('hidden');let xs=await api('/api/files?path='+encodeURIComponent(p));q('#list').innerHTML=xs.map(x=>'<div class="row '+(x.directory?'folder':'')+'"><a href="#" data-path="'+escapeHtml(x.path)+'" data-directory="'+x.directory+'">'+(x.directory?'[folder] ':'')+escapeHtml(x.path.split('/').pop())+'</a><span class="muted">'+(x.latest?x.latest.size+' bytes, '+new Date(x.latest.created).toLocaleString():'')+'</span></div>').join('')||'<p>No files in this folder.</p>';q('#list').querySelectorAll('a[data-path]').forEach(a=>a.onclick=e=>{e.preventDefault();openItem(a.dataset.path,a.dataset.directory==='true')})}
